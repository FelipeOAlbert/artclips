...
win.add(topView);
	...
	topView.add(questionView);
		...
		var question_text = Ti.UI.createLabel({
			text: 'Procurando por qual tipo de im\363vel?',
			font: {
				fontSize: 40,
				fontFamily: 'Georgia'
			},
		...
		var aCheck = Ti.UI.createView({
			width: 35,
			height: 35,
			top: 50+question_text.height+question_sub.height,
			left: 40,
		...
		var bCheck = Ti.UI.createView({
			width: 35,
			height: 35,
			top: 50+question_text.height+question_sub.height+aLabel.height+15,
			left: 40,
			backgroundImage: '../images/normal_Slt.png',
			selected: false,
			label: 'Apartamento'
...
voltar_btn.addEventListener('click', function(e) {
	...
	Ti.App.fireEvent('win_03',{rg:win.rg, esc:win.esc, prof:win.prof});
...
avancar_btn.addEventListener('click', function(e) {
	...
	if (aCheck.selected == true && bCheck.selected == false)
	{
		...
		Ti.App.fireEvent('win_05',{rg:win.rg, esc:win.esc, prof:win.prof, tempProcura:win.tempProcura, tipoImovel:aLabel.text});
	}
	...
	else if (aCheck.selected == false && bCheck.selected == true)
	{
		...
		Ti.App.fireEvent('win_05',{rg:win.rg, esc:win.esc, prof:win.prof, tempProcura:win.tempProcura, tipoImovel:bLabel.text});
	}
	...
	else if (aCheck.selected == true && bCheck.selected == true)
	{
		...
		Ti.App.fireEvent('win_05',{rg:win.rg, esc:win.esc, prof:win.prof, tempProcura:win.tempProcura, tipoImovel:aLabel.text+' ou '+bLabel.text});
	}
