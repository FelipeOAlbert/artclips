...
class Ipad_model extends CI_Model {
	function __construct()
	{
		parent::__construct();
	}
	
	function checkAuthIpadNew($options = array()){
		if(!isset($options['id_ipad'])) return false;
		
		$existe = $this->db->select('active')
						->from(DB_DEFAULT . '.event_ipad_activated ipad')
						->where('ipad.id_ipad_login', $options['id_ipad'])						
						->where('ipad.active', 1)
						->count_all_results();
		return (intval($existe)>0?true:false);
	}
	
	function saveAuthIpadNew($options = array()){

		if( isset($options['name']) && 
			isset($options['login']) && 
			isset($options['udid']) && 
			isset($options['id_ipad']) ){
				
				$this->db->set('id_ipad_login', (int) $options['id_ipad']);
				$this->db->set('udid', (string) $options['udid']);
				$this->db->set('active', 1);
				
				$sefoi = $this->db->insert(DB_DEFAULT . '.event_ipad_activated');
	...
	function authIpad( $options = array() )
	{
		if ( isset($options['login']) && isset($options['password']) && isset($options['udid']) && isset($options['name']) ){
			return $this->db->select('ipad.id_ipad as id_ipad, e.id_event AS id_evento, e.id_user AS id_usuario, e.name AS nome, e.active AS ativo')
						->from(DB_DEFAULT . '.event_ipad ipad')
						->join(DB_DEFAULT . '.event e', 'ipad.id_event = e.id_event')
						->where('ipad.login_ipad', $options['login'])
						->where('ipad.senha_ipad', MD5($options['password']) )
						->get()->result();	
		}else{
			return array();
		}	
